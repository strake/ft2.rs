...
impl Stroker {
    ...
    pub fn set(&self, radius: ffi::FT_Fixed, line_cap: StrokerLineCap, line_join: StrokerLineJoin, miter_limit: ffi::FT_Fixed) {
        unsafe { ffi::FT_Stroker_Set(self.raw, radius, line_cap as u32, line_join as u32, miter_limit); }
    }

    pub fn raw(&self) -> &ffi::FT_StrokerRec { unsafe { &*self.raw } }

    pub fn raw_mut(&mut self) -> &mut ffi::FT_StrokerRec { unsafe { &mut *self.raw } }

    pub(crate) fn raw_stroker(&self) -> ffi::FT_Stroker { self.raw }
}
